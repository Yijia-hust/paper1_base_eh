...
% 采用确定性单预测场景，不再需要误差范围；两座风场容量分别为1000 MW和500 MW
PW(1,:) = min(PW(1,:),1000);
PW(2,:) = PW(2,:)*500/max(PW(2,:));

%% 制氢站HGS参数
eta_p2h=16.67;  %(电解水制氢效率，单位：kg/MWh)
eta_h2c=0.0008; %(压缩氢气耗电效率，单位：MWh/kg)
% 20260711增加，并对应的在约束中做限制
Qgen_max = 2000;       % HGS每小时最大制氢量 kg
QDT_hour_max = 3000;   % HGS每小时向全部车辆装氢总量 kg

PHmin=0;
PHmax=Qgen_max/(eta_p2h*delta_t) + Qgen_max*eta_h2c/delta_t;  % 原来直接写800，过大
...
Q_init=SOC_init*QN;
Qin_max=1000;     % 储氢罐进气量的上限(每小时)
Qout_max=1000;    % 储氢罐出气量的上限(每小时)



% 假设HGS有6台运氢车DT(假设只有1个HGS)
N_dt=6;
...
% 最大车载氢容量（也是HGS装载量的上限）
Q_dt_cap = 1000;  % 要不设高一点，我发现每辆车出发时都装满了
% HGS装载量下限
Q_dt_min = 0;


M_time=30;          % 时间约束
M_h2=Q_dt_cap;      % 氢量约束，若单车容量1000，则为1000
...
target_h2_load = 5000;
HLC_load = HLC_load*target_h2_load/sum(HLC_load(:));  % 6辆车且每车一天只跑一趟，理论运输上限为6000 kg；将负荷设为5000 kg可留出路线和时序余量。
% 该负荷是我的缩小算例，不是论文原来的26311 kg。论文原来可能40辆车，我只设6辆。
